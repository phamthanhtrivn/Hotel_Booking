...
export default function CustomerManagement() {
  ...
  const columns = [
    { key: "maTaiKhoan", label: "ID" },
    {
      ...
      label: "Họ và tên",
      render: (item) =>
        item?.khachHang?.hoTenKH ? item.khachHang.hoTenKH : "",
    },
    {
      key: "soDienThoai",
      label: "Số điện thoại",
      render: (item) =>
        item?.khachHang?.soDienThoai ? item.khachHang.soDienThoai : "",
    },
    {
      key: "diemTichLuyKH",
      label: "Điểm tích lũy",
      render: (item) => item?.khachHang?.diemTichLuy ?? 0,
    },
    {
      key: "email",
      label: "Email",
    ...
    const deleteCustomer = async () => {
      try {
        ...
        setCustomers((prev) =>
          prev.filter(
            (c) =>
              c.khachHang.maKhachHang !== currentCustomer.khachHang.maKhachHang
          )
        );
      } catch (err) {
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            <Input
              ...
              onChange={(e) =>
                setFliter({ ...fliter, keyword: e.target.value })
              }
              placeholder="Tìm kiếm khách hàng, email, số điện thoại..."
            />
            <Select
              value={fliter.trangThai}
              onValueChange={(value) =>
                setFliter({ ...fliter, trangThai: value })
              }
            >
            ...
            <Select
              value={fliter.sortField}
              onValueChange={(value) =>
                setFliter({ ...fliter, sortField: value })
              }
            >
              <SelectTrigger className="w-full cursor-pointer">
              ...
              <SelectContent>
                <SelectItem value="khachHang.diemTichLuy&asc">
                  Điểm tăng dần
                </SelectItem>
                <SelectItem value="khachHang.diemTichLuy&desc">
                  Điểm giảm dần
                </SelectItem>
                <SelectItem value="khachHang.hoTenKH&asc">
                  Tên tăng dần
                </SelectItem>
                <SelectItem value="khachHang.hoTenKH&desc">
                  Tên giảm dần
                </SelectItem>
              </SelectContent>
            </Select>
